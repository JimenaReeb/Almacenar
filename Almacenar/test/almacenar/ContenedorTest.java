...
public class ContenedorTest {
    ...
    public void testAgregar() {
        ...
        contenedor.limpiar();
        for (int i = 0; i < cantidadPaquetes; i++) {
            assertFalse("Fallo al agregar un paquete al contenedor", contenedor.agregar(paquetes[i]));
            assertFalse("Debe fallar al agregar dos veces el mismo paquete", contenedor.agregar(paquetes[i]));
            assertFalse("Fallo al verificar si contiene el paquete", contenedor.agregar(paquetes[i]));
        }
    }
    ...
    public void testRemover() {
        ...
        for (int i = 0; i < cantidadPaquetes; i++) {
            ...
            for (int j = 0; j < i; j++) {
                ...
                assertFalse("Fallo si es contenedor aùn tiene el paquete despues de removerlo", contenedor.contiene(paquetes[j]));
            }
            for (int j = 0; j < cantidadPaquetes; j++) {
                assertFalse("Debe fallar al intentar remover un paquete que no està en la lista", contenedor.remover(paquetes[j]));
            }
        }
    }
